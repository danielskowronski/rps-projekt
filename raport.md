...
(2)
Badamy ilość adresów IP z jakich logował się każdy użytkownik. Ponieważ jest to zjawisko naturalne stawiamy hipotezę iż rozkład tych wartości jest normalny. Dane obcinamy metodą 1.5 IQR z góry celem odrzucenia użytkowników którym się chyba nudzi i studiują za długo, natomiast z dołu stałą=2 (ponieważ dwa adresy IP wyróżniają się jeśli chodzi o częstość autoryzacji - stąd wniosek o tym że są to adresy IP uczelni) - odrzuci to użytkowników którzy prawdopodobnie zalogowali sie tylko z uczelni (*-> może by wyciąć takowych jakąś turbo fajną agregacją? w SQLu zrobiłbym to szybko ale w R...) - 1.5IQR nie obcinałoby od dołu żadnych wartości (granica<0).
